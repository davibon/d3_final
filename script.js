...
function bind(dataSet, textArray) {
    //4.2 Stacked-to-Grouped bind()    
    if (textArray.length < 2) {
        textArray.length = 1;
    }
    console.log("textArray1" + textArray);
    ...
    arrd3.enter().append("g")
        ...
        .on("mouseover", function (d, i) {
            ...
            if (textArray.length > 1) {
                ...
                var textIndex = causes.indexOf(textArray);
                console.log("textArray" + textArray);
                console.log("textIndex" + textIndex);
                text = causesCHT[textIndex];
            }
            d3.select("#tooltip").select("#type").text("類別:" + text);
        });
    arrd3.exit().remove();
